...
func TestParseApprovalCommandBuildsResponse(t *testing.T) {
	...
	response, err := ParseApprovalCommand("approval-1 always", now)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := ParseApprovalCommand("approval-1 nope", now); err == nil || !strings.Contains(err.Error(), "unknown approval choice") {
